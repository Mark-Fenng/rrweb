      ...
      for (const name in n.attributes) {
        ...
        if (!name.startsWith('rr_')) {
          ...
          if (isTextarea || isRemoteOrDynamicCss) {
            ...
            for (const c of Array.from(node.childNodes as (INode | RRNode)[])) {
              if (c.nodeType === node.TEXT_NODE) {
                node.removeChild(c as INode & RRNode);
              }
            }
            node.appendChild(child as INode & RRNode);
            continue;
          }
          ...
          if (tagName === 'canvas' && name === 'rr_dataURL') {
            const image = doc.createElement('img') as
              | HTMLImageElement
              | RRImageElement;
            image.src = value;
            image.onload = () => {
          ...
          if (name === 'rr_width') {
            (node as HTMLElement | RRElement).style.setProperty('width', value);
          }
          if (name === 'rr_height') {
            (node as HTMLElement | RRElement).style.setProperty(
              'height',
              value,
            );
          }
          if (name === 'rr_mediaCurrentTime') {
            (node as RRMediaElement).currentTime = n.attributes
              .rr_mediaCurrentTime as number;
          }
          if (name === 'rr_mediaState') {
            switch (value) {
              case 'played':
                (node as RRMediaElement)
                  .play()
                  .catch((e) => console.warn('media playback error', e));
                break;
              case 'paused':
                (node as RRMediaElement).pause();
                break;
              default:
            }
          }
        ...
        if (!node.shadowRoot) {
          ...
          while (node.shadowRoot.firstChild) {
            node.shadowRoot.removeChild(
              node.shadowRoot.firstChild as Node & RRNode,
            );
          }
        }
      }
...
export function buildNodeWithSN(
  ...
  if (n.rootId) {
    console.assert(
      ((map[n.rootId] as unknown) as RRDocument) === doc,
  ...
  (node as INode | RRNode).__sn = n;
  map[n.id] = node as INode | RRNode;
  ...
  if (
    ...
    for (const childN of n.childNodes) {
      ...
      if (childN.isShadow && isElement(node) && node.shadowRoot) {
        node.shadowRoot.appendChild(childNode as RRNode);
      } else {
        node.appendChild(childNode as RRNode & Node);
      }
      if (afterAppend) {
  ...
  return node as INode | RRNode;
}

function visit(idNodeMap: idNodeMap, onVisit: (node: INode | RRNode) => void) {
  function walk(node: INode | RRNode) {
    onVisit(node);
  }
...
function handleScroll(node: INode | RRNode) {
  const n = node.__sn;
  if (n.type !== NodeType.Element) {
    return;
  }
  const el = node as RRElement;
  for (const name in n.attributes) {
